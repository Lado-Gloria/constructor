...
class Rental {
    ...
    calculateRentalDuration() {
      const oneDay = 24 * 60 * 60 * 1000; // Convert milliseconds to days
  ...
  const cars = new Car('Toyota', 'Corolla', 2018, true);
  const rentalStartDates = new Date('2023-05-01');
  const rentalEndDates = new Date('2023-05-07');
  const rentals = new Rental(car, 'John Smith', rentalStartDates, rentalEndDates);
  
  console.log(`The car ${car.make} ${car.model} is available for rent: ${car.isAvailable}`);
  car.toggleAvailability();
  console.log(`The car ${car.make} ${car.model} is available for rent: ${car.isAvailable}`);
...
const rentalStartDate = new Date("2023-05-01");
const rentalEndDate = new Date("2023-05-05");
const rental = new Rental(car1, "John Doe", rentalStartDate, rentalEndDate);
const rentalDuration2 = rental.calculateRentalDuration();
console.log("Rental duration: " + rentalDuration2 + " days");
...
class Question {
  ...
  const question =new Question("String","Array","the correct answer to the question")
...
// score if the answer is correct.
  ...
  const q2 = new Question(
    ...
    ['Mount Kilimanjaro', 'Mount Everest', 'Mount Denali', 'Mount Fuji'],
    'Mount Everest'
  );
  ...
  quiz.submitAnswer('Mount Denali'); 
